...
export default function SingleProductPage() {
  ...
  const handleAdd = async () => {
    ...
    // 🧠 Validate material next
    if (!selectedMaterial) {
      await Swal.fire({
        icon: "warning",
        title: "Material Required 🧵",
        text: "Please select a material before adding to cart.",
        confirmButtonText: "OK",
    ...
    const itemToAdd = {
      quantity: selectedPack ? null : quantity, // ❌ null if pack selected
      name: product.name,
      ...
      pack: selectedPack || null, // ✅ store pack separately
      material: selectedMaterial || null,
      type: selectedType || null,
      smart_card: selectedSmartCard || null,
    };
          ...
          <div className="lg:w-1/2 space-y-4">
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              <span className="text-3xl font-semibold text-sky-600">
                AED{Number(packPrice ?? product.sale_price).toFixed(2)}
              </span>
                  ...
                  {product.packs.map((pack, idx) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        // 👇 Convert pack (string) to number and multiply by base price
                        const packQty = Number(pack);
                        const basePrice = Number(product.sale_price);
                        setPackPrice(basePrice * packQty);
                      }}
                      className={`border rounded-[40px] py-[10px] px-[20px] capitalize transition-all ${
                        selectedPack === pack
                          ? "bg-sky-600 border-sky-600 text-white"
